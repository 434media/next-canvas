      ...
      {isDark && (
        <div className="absolute inset-0 overflow-hidden opacity-10">
          <motion.div
            ...
            className="absolute right-20 top-20 h-96 w-96 rounded-full bg-[#ffd23f]"
          />
          <div className="absolute bottom-0 left-0 font-[var(--font-menda-black)] text-[300px] text-[#ff6b35]">$</div>
        </div>
      )}
      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        <motion.div
          ...
          className={`overflow-hidden rounded-2xl p-8 md:p-12 ${
            isDark
              ? "border-4 border-[#00ff88] bg-gradient-to-br from-[#1a1a1a] via-[#121212] to-[#0a0a0a]"
              : "bg-white shadow-2xl"
          }`}
        ...
        >
            ...
            {isDark ? (
              ...
              >
                <div className="mb-4 flex items-center gap-4">
                  <div className="flex h-16 w-16 items-center justify-center rounded-full border-4 border-[#ff6b35] bg-[#0a0a0a]">
                    <CreditCard className="text-[#ff6b35]" size={32} />
                  </div>
                  <Zap className="text-[#ffd23f]" size={40} />
                </div>
                <h2 className="mb-2 font-[var(--font-menda-black)] text-5xl tracking-tighter text-[#ffd23f] md:text-6xl">
                  LOCK IT IN
                </h2>
                <div className="inline-block border-b-8 border-[#00ff88] pb-2">
                  <p className="font-[var(--font-menda-black)] text-2xl text-[#f5f5f5]">Secure Your Workshop Date</p>
                </div>
              </motion.div>
            ) : (
          ...
          <p className={`mb-8 text-lg leading-relaxed ${isDark ? "text-[#a3a3a3]" : "text-gray-700"}`}>
            {isDark ? (
              <>
                Ready to roll? <span className="font-bold text-[#ff6b35]">Lock in your date</span> with a secure
                retainer payment. We handle the rest —{" "}
                <span className="font-bold text-[#ffd23f]">proposal, agreement, the whole nine</span>.
              </>
            ) : (
              <>
          ...
          <div className="mb-8 space-y-4">
            {benefits.map((item, index) => (
              ...
              >
                <CheckCircle
                  className={`mt-1 flex-shrink-0 ${isDark ? "text-[#00ff88]" : "text-green-600"}`}
                  size={24}
                />
          ...
          <div
            className={`rounded-xl p-6 ${
              isDark ? "border-2 border-[#ff6b35] bg-[#0a0a0a]" : "bg-gradient-to-r from-[#2563eb]/5 to-[#1e40af]/5"
            }`}
          >
            <div className="flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
              <div className="text-center sm:text-left">
                <p className={`mb-1 font-bold text-sm ${isDark ? "text-[#ffd23f]" : "text-gray-600"}`}>
                  {isDark ? "SECURE PAYMENT VIA" : "Powered by"}
                </p>
                <div className="flex items-center gap-2">
                  <Shield className={isDark ? "text-[#00ff88]" : "text-[#2563eb]"} size={24} />
